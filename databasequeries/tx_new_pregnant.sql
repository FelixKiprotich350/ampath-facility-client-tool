-- =========================================================
-- TX_NEW - Pregnant (Starting on ART)
-- =========================================================
...
COUNT(DISTINCT h.patient_id) AS tx_new_pregnant
FROM kenyaemr_etl.etl_hiv_enrollment h
JOIN kenyaemr_etl.etl_patient_demographics d ON h.patient_id=d.patient_id
JOIN kenyaemr_etl.etl_patient_hiv_followup f ON h.patient_id=f.patient_id
WHERE h.visit_date BETWEEN @start_date AND @end_date
AND d.gender='F'
AND f.pregnancy_status=1065
AND f.visit_date BETWEEN DATE_SUB(h.visit_date, INTERVAL 30 DAY) AND h.visit_date
