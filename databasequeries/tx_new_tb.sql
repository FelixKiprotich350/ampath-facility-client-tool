...
COUNT(DISTINCT h.patient_id) AS tx_new_tb
FROM kenyaemr_etl.etl_hiv_enrollment h
JOIN kenyaemr_etl.etl_patient_demographics d ON h.patient_id=d.patient_id
JOIN kenyaemr_etl.etl_tb_enrollment t ON h.patient_id=t.patient_id
WHERE h.visit_date BETWEEN @start_date AND @end_date
AND t.visit_date BETWEEN DATE_SUB(h.visit_date, INTERVAL 90 DAY) AND DATE_ADD(h.visit_date, INTERVAL 30 DAY)
