-- On DTG (Patients currently on DTG-based regimen)
...
FROM (
    ...
    FROM (
        select t.patient_id
        from (select de.patient_id,
                     mid(max(concat(de.date_started, de.regimen_name)), 11) as current_regimen
              from kenyaemr_etl.etl_drug_event de
                       join kenyaemr_etl.etl_patient_hiv_followup f on de.patient_id = f.patient_id
              where de.program = 'HIV'
                and date(de.date_started) <= date('2025-10-31 00:00:00')
                and f.visit_date between date('2025-10-01 00:00:00') and date('2025-10-31 00:00:00')
              group by de.patient_id) t
        where t.current_regimen like '%DTG%'
    ) c
